...
SETUPSPRITES                           
                ...
                STA $D02D
                LDA #$08                 ;-> SPRITE 7 (MISSILE) COLOR = ORANGE
                STA $D02E
                ...
                LDA #$92                ; MISSILE
                STA $07FF
                LDA #%00111111         ; TURN ON 7 SPRITES - EXPLOSION NOT INCLUDED
                STA $D015
                RTS
...
DRAWTEXTSCREEN
                ...
                LDX #$00
LOOPTEXT        LDY #$00
INNERLOOPY      LDA ($FB),Y     ; LOOPING W/ ZERO PAGE ADDRESSES 
...
CLEANCOLOR       DEX 
                 ...
                 BNE CLEANCOLOR
                 LDX #$00
TLOOP            LDA ARENATEXT,X        ; WRITES THE TOPMOST LINE
...
WAITRASTER                 
                LDA #20         ; WAITS FOR RASTERLINE 20
                CMP $D012
                BNE WAITRASTER
                RTS
...
FIRE            LSR A                     ;READ FIRE
                ...
                CMP #$01                  ;IF 1, THEN BUTTON LOCKED UP
                BEQ NOJOY                 
...
;-------------------------ENEMYSHIP MOVEMENT-------------------------------------
ENEMYSHIP 
                LDA ENSHIPDIR         ; CHECK ENEMYSHIP DIRECTION      
                CMP #$00              ; IF 1, THEN USE SINEX1 TABLE
                BNE R2L
                LDX BYTECOUNT         ; OTHERWISE USE SINEX0 TABLE
                ...
                INC BYTECOUNT
                RTS
R2L             LDX BYTECOUNT         ; USE SINEX1 TABLE
...
GOUP            LDX SPRITEPOS+$09     
                ...
                CPX #$90              ; IF REACHED $90 (UPPER PART OF THE LIMIT)
...
;------------------------MISSILE---------------------------------
MISSILE
                LDX MISSILETIME         ; CHECK MISSILETIME
                CPX #00
                BEQ FINMISSILE          ; IF ZERO, NO MISSILE, FINALIZE
                LDX MISSILEON           ; IS THERE A MISSILE ON SCREEN?
                CPX #01
                BEQ MOVEMISSILE         ; IF YES, GO TO MOVE ROUTINE
                LDX MISSILEXNO          ; WE ARE PUTTING A NEW MISSILE ON SCREEN
                LDA MISSILEX,X          ; THE NEXT ROW IS TRANSFERED TO X POSITION
                STA SPRITEPOS+$0E                
                LDY MISSILEY            ; STARTING MISSILE Y POS IS READ 
                INC MISSILEON           ; NOW THERE IS A MISSILE ON SCREEN
                LDA $D015               ; ACTIVATE MISSILE SPRITE
                EOR #%10000000
                STA $D015
                JSR MISSILEBEEP
                JMP STOREMISSILE
MOVEMISSILE     LDX MISSILEXNO
                LDA MISSILEX,X
                STA SPRITEPOS+$0E
                LDY SPRITEPOS+$0F       ; LOAD Y POSITION OF MISSILE
                INY                     ; INCREMENT Y POS
                CPY #$FF                ; COMPARE TO $E5 (BOTTOM LIMIT)
                BCC STOREMISSILE        ; IF LESS, THEN GO TO STOREMISSILE AND EXIT
                INC MISSILEXNO          ; REACHED THE BOTTOM, SO INCREMENT X POS FROM TABLE       
                LDA MISSILEXNO          ; CHECK X POS TABLE 
                CMP #$05                ; REACHED 5?
                BCC STOREMISSILE        ; NO, CONTINUE
                LDA #$00                ; YES, MAKE IT ZERO
                STA MISSILEXNO 
STOREMISSILE    STY SPRITEPOS+$0F       ; STORE Y POSITION OF MISSILE
FINMISSILE      RTS
...
COLLIDED
                ...
                LDA #%00111111        ; RETURN BACK TO ORIGINAL POSITIONS EXCEPT MAIN SHIP
...
COLLIDED2
                ...
                LDA #%00111111        ; 7 SPRITES ARE ON (AGAIN)  
                STA $D015           
                LDA #$00              ; RESET COLLISION REGISTER
                STA $D01E      
...
;-------------------END GAME------------------------------
GAMEOVER
                LDX #0                  ; GAME OVER TEXT
GO              LDA ENDTEXT1,X
                STA SCREEN3,X
                INX
                CPX #9
                BNE GO
                LDX #0                  ; COLOR OF GAME OVER TEXT
GO2             LDA #1
                STA SCREEN3COL,X
                INX
                CPX #9
                BNE GO2
                LDX #0                  ; HIT SPACE TO CONTINUE TEXT
GO3             LDA ENDTEXT2,X
                STA SCREEN4,X
                INX
                CPX #21
                BNE GO3
                LDX #0                  ; COLOR OF HIT SPACE TO CONTINUE TEXT
GO4             LDA #1
                STA SCREEN4COL,X
                INX
                CPX #25
...
INPUT           JSR COLORANIM1          ; TEXT COLOR ANIMATIN OF THE 1ST LINE
                JSR COLORANIM2          ; TEXT COLOR ANIMATIN OF THE 2ND LINE
                JSR $FFE4               ; WAIT FOR KEYBOARD INPUT
                BEQ INPUT
                STA KEY
                CMP #$20                ; IS SPACE HIT?
                BNE INPUT       
...
CHECKEXTRALIFE 
                LDA SCORE
                CMP #00
                BNE ENEMYHIT             ; WRITE "ENEMY HIT" TEXT IF NO EXTRA LIFE
                LDA SCORE+1
                CMP #01
                ...
                BEQ ENEMYHIT            ; FOR 500,  NO EXTRA LIFE                  
                CMP #08
                BEQ ADDEXTRALIFE        ; FOR 800,  ADD AN EXTRA LIFE
                JMP FINALEXTRALIFE
...
EXT             LDA EXTRALIFETEXT,X      ; ON THE BOTTOM OF THE SCREEN
                STA SCREEN6,X
                INX
                CPX #40
                BNE EXT          
...
;----------------------COLLISION ANIMATIN-------------------------------
COLLANIM
                JSR EXPANDSPRITEPOS    ; CHECK THE FINAL SPRITE POSITIONS
                LDX #$00
EXPLPLR         LDA EXPLTBL,X          ; GET THE EXPLOSION SPRITE POINTERS
                STA $07FE              ; FROM THE TABLE
                JSR DELAYANIM          ; INSERT A DELAY BETWEEN SPRITES
                INX
                CPX #12                ; TOTAL 12 SPRITES FOR ONLY 
                BNE EXPLPLR            ; AN EXPLOSION ANIMATION
                RTS                    ; I AM TOO GENEROUS :)
DELAYANIM       LDY #8                 ; DELAY ROUTINE 
WAIT1              
WAITVB          BIT $D011
                BPL WAITVB
WAITVB2         BIT $D011
...
CHECKHISCORE
                ...
                BCC YES          ; IF SCORE < HISCORE, GO AND FORM THE NEW HIGH SCORE
...
ENSHIPDIR       BYTE 0             ; ENEMY SHIP CIRCLE DIRECTION   0= LEFT2RIGHT, 1=RIGHT2LEFT
...
ENEMY2DIR       BYTE $00           ; DIRECTION OF ENEMY SHIP 2 (UP OR DOWN)
LIVES           BYTE $00           ; NUMBER OF LIVES
KEY             BYTE 0             ; GET CHAR. OF KEY PRESSED
ATTDEC  BYTE 0                     ; SOUND -> ATTACK-DECAY VARIABLE
SUSREL  BYTE 0                     ; SOUND -> SUSTAIN-RELEASE VARIABLE
VOLUME  BYTE 0                     ; SOUND -> VOLUME VALUE
HIFREQ  BYTE 0                     ; SOUND -> HIGH FREQUENCY VARIABLE
LOFREQ  BYTE 0                     ; SOUND -> LOW FREQUENCY VARIABLE
WAVEFM  BYTE 0                     ; SOUND -> WAVEFORM VARIABLE
...
ENDTEXT2        TEXT 'hit space to continue'

MAINSHIPHIT     TEXT '    you are hit - be careful please     '
ENEMYSHIPHIT    TEXT '   you hit the enemy ship, good work    '
EXTRALIFETEXT   TEXT '  bravo! you've earned an extra life!   '

SCROLLTEXT      TEXT 'a-space-game    design and programming :'   
...
MISSILETIME     BYTE 0
MISSILEON       BYTE 0
MISSILEX        BYTE 80,105,130,30,55
MISSILEY        BYTE 30        
MISSILEXNO      BYTE 00
...
*=$4800
        ...
        BYTE    $20,$56,$20,$20,$09,$0E,$20,$14,$08,$09,$13,$20,$07,$01,$0D,$05,$2C,$20,$13,$0F,$20,$02,$05,$20,$10,$12,$05,$10,$01,$12,$05,$04,$2E,$2E,$2E,$20,$20,$20,$20,$20
        BYTE    $20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20
        BYTE    $20,$58,$56,$20,$10,$0C,$05,$01,$13,$05,$20,$15,$13,$05,$20,$0A,$0F,$19,$13,$14,$09,$03,$0B,$20,$0F,$0E,$20,$10,$0F,$12,$14,$20,$14,$17,$0F,$20,$56,$20,$20,$20
        BYTE    $20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$56,$20,$20,$20,$20,$58,$20,$20
        BYTE    $20,$20,$20,$20,$19,$0F,$15,$20,$17,$09,$0C,$0C,$20,$08,$01,$16,$05,$20,$01,$0E,$20,$05,$18,$14,$12,$01,$20,$0C,$09,$06,$05,$20,$06,$0F,$12,$56,$20,$20,$20,$56
        BYTE    $20,$20,$20,$20,$05,$01,$03,$08,$20,$32,$30,$30,$20,$10,$0F,$09,$0E,$14,$13,$2C,$20,$02,$15,$14,$20,$02,$05,$17,$01,$12,$05,$20,$0F,$06,$20,$01,$0E,$20,$20,$20
        BYTE    $20,$20,$20,$20,$01,$04,$04,$09,$14,$09,$0F,$0E,$01,$0C,$20,$0D,$09,$13,$13,$09,$0C,$05,$20,$21,$21,$21,$20,$20,$20,$56,$20,$20,$20,$56,$20,$20,$20,$56,$20,$20
        BYTE    $20,$20,$56,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20,$20
...
*=$4C00 
        ...
        BYTE    $00,$0F,$00,$00,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$0F,$0F,$0F,$0F,$01
        BYTE    $00,$00,$00,$00,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$01,$05,$05,$0F,$01
        BYTE    $00,$01,$01,$00,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$01,$01,$01,$01,$01
        BYTE    $00,$00,$00,$00,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$07,$01,$01,$01,$01,$01,$0E,$0F,$01
        BYTE    $00,$00,$00,$00,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$01,$01,$01,$0F,$01
        BYTE    $00,$00,$00,$00,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$0F,$0F
        BYTE    $0F,$0F,$0F,$0F,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$05,$00,$00,$00,$0E,$01,$01,$01,$01,$05,$05,$05,$01,$05,$01
        BYTE    $00,$00,$0E,$00,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F
        BYTE    $0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$0F,$00,$00,$00,$00,$00,$00,$00,$00,$07,$00,$00,$01,$00,$00,$00,$01,$00,$01,$01,$0F,$0D,$0D,$0D,$0D,$0D,$0F,$01
        ...
        BYTE    $00,$00,$00,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$0A,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$01,$01
        BYTE    $00,$0E,$00,$02,$02,$02,$02,$0A,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$0A,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$02,$0A,$02
